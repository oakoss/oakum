...
fn local_tags(root: &Path) -> String {
    git_stdout(root, &["tag", "--list"])
}

fn commit_at(root: &Path, reference: &str) -> String {
    git_stdout(root, &["rev-parse", &format!("{reference}^{{commit}}")])
}

fn assert_annotated(root: &Path, name: &str) {
    assert_eq!(git_stdout(root, &["cat-file", "-t", name]), "tag", "{name}");
}

fn pending_demo(label: &str) -> Fixture {
...
fn a_second_push_url_over_ssh_still_gets_the_note() {
    ...
    add_bare_origin(&root);
    let bare = sibling(&root, "pushurl-many.git");
...
fn an_identical_note_owed_by_both_directions_is_said_once() {
    ...
    let scratch = sibling(&root, "note-direction");
...
fn the_ssh_transport_is_read_once_however_many_remote_children_run() {
    ...
    let scratch = sibling(&root, "probe-once-scratch");
...
fn current_tag_behind_head_is_resumed() {
    ...
    assert_ne!(
        git_stdout(&root, &["rev-parse", "v0.1.1^{}"]),
        git_stdout(&root, &["rev-parse", "HEAD"])
    );
}
...
fn a_blocking_signing_program_meets_the_deadline() {
    ...
    add_bare_origin(&root);
    let gpg = sibling(&root, "deadline-signing-gpg");
...
fn a_tag_that_shares_a_branch_name_pushes_the_tag_ref() {
    ...
    create.assert();
    let listed = git_stdout(&root, &["ls-remote", "origin"]);
    assert!(listed.contains("refs/tags/v0.1.1"), "{listed}");
    assert!(
...
fn a_push_that_lands_but_dies_reports_pushed() {
    ...
    add_bare_origin(&root);
    let shim_dir = sibling(&root, "push-dies-shim");
    fs::create_dir_all(&shim_dir).expect("shim dir");
    let real = String::from_utf8(
        Command::new("sh")
            .args(["-c", "command -v git"])
...
fn a_dead_push_whose_reread_fails_is_unverified() {
    ...
    add_bare_origin(&root);
    let marker = sibling(&root, "push-dies-blind-marker");
    let _ = fs::remove_file(&marker);
    let shim_dir = sibling(&root, "push-dies-blind-shim");
    fs::create_dir_all(&shim_dir).expect("shim dir");
    let real = String::from_utf8(
        Command::new("sh")
            .args(["-c", "command -v git"])
...
fn a_shallow_clone_refuses_before_the_unread_tags_scan() {
    ...
    commit(&root, "second");
    git(&root, &["tag", "v0.1.0"]);
    let clone = sibling(&root, "shallow");
    git(
        &root,
        &[
            "-c",
...
fn a_corrupt_tag_object_is_unverified_not_nothing() {
    ...
    let object = git_stdout(&root, &["rev-parse", "refs/tags/v0.1.0"]);
    fs::remove_file(
...
fn the_tag_push_refuses_prompts() {
    ...
    let bare = sibling(&root, "push-batchmode.git");
    ...
    let log = sibling(&root, "push-batchmode-ssh.log");
    let _ = fs::remove_file(&log);
    let script = local_ssh_transport(root.container(), &bare, &log);
...
fn release_body_is_the_changelog_section_for_the_version() {
    ...
    create.assert();
    assert!(
        !stdout_of(&out).contains("release body:"),
        "{}",
        stdout_of(&out)
    );
}
...
#[test]
fn release_body_falls_back_to_the_title_and_says_so() {
    let root = pending_demo("body-no-changelog");
    add_bare_origin(&root);
    git(&root, &["tag", "v0.1.1"]);
    git(&root, &["push", "origin", "refs/tags/v0.1.1"]);
    git(&root, &["tag", "-d", "v0.1.1"]);
    ...
    let stderr = stderr_of(&out);
    assert!(
        stderr.contains("release body: no CHANGELOG.md at ")
            && stderr.contains("; using the title"),
        "{stderr}"
    );
    assert!(
        !stdout_of(&out).contains("release body:"),
...
fn mock_create_with_body<'a>(server: &'a MockServer, body_fragment: &str) -> httpmock::Mock<'a> {
    let needle = format!("\"body\":\"{body_fragment}\"");
    server.mock(|when, then| {
        when.method(POST)
            .path("/repos/oakoss/oakum/releases")
            .body_includes(needle);
        then.status(201).json_body(json!({
            "html_url": "https://github.com/oakoss/oakum/releases/tag/v0.1.1"
        }));
    })
...
#[test]
fn resumed_release_body_comes_from_the_tagged_commit() {
    let root = pending_demo("body-resumed");
    fs::write(
        root.join("CHANGELOG.md"),
        "# Changelog\n\n## 0.1.1 (2026-09-08)\n\n- from the tag\n",
    )
    ...
    .expect("changelog");
    commit(&root, "later edit");
    let server = MockServer::start();
    mock_lookup_empty(&server, "v0.1.1");
    let create = mock_create_with_body(&server, "- from the tag\\n");
    let out = release_cmd(&root, &server);
    assert!(
        out.status.success(),
        "{}{}",
...
#[test]
fn empty_section_falls_back_to_the_title_and_says_so() {
    let root = pending_demo("body-empty-section");
    fs::write(
        root.join("CHANGELOG.md"),
        "# Changelog\n\n## 0.1.1 (2026-09-08)\n\n## 0.1.0 (2026-09-01)\n\n- first\n",
    )
    ...
    let server = MockServer::start();
    mock_lookup_empty(&server, "v0.1.1");
    let create = mock_create_with_body(&server, "demo 0.1.1\\n");
    let out = release_cmd(&root, &server);
    assert!(
        out.status.success(),
        "{}{}",
    ...
    assert!(
        stderr_of(&out)
            .contains("release body: CHANGELOG.md has an empty 0.1.1 section; using the title"),
        "{}",
        stderr_of(&out)
    );
}
...
#[test]
fn prerelease_body_matches_its_own_section() {
    let root = temp_git_repo("body-prerelease");
    cargo_package(&root, "demo", "0.1.0");
    commit(&root, "init");
    git(&root, &["tag", "v0.1.0"]);
    cargo_package(&root, "demo", "0.1.1-rc.1");
    fs::write(
        root.join("CHANGELOG.md"),
    ...
    commit(&root, "version");
    add_bare_origin(&root);
    git(&root, &["tag", "v0.1.1-rc.1"]);
    git(&root, &["push", "origin", "refs/tags/v0.1.1-rc.1"]);
    git(&root, &["tag", "-d", "v0.1.1-rc.1"]);
    let server = MockServer::start();
    mock_lookup_empty(&server, "v0.1.1-rc.1");
    let create = server.mock(|when, then| {
        when.method(POST)
            .path("/repos/oakoss/oakum/releases")
            .body_includes("\"body\":\"- rc note\\n\"");
        then.status(201).json_body(json!({
            "html_url": "https://github.com/oakoss/oakum/releases/tag/v0.1.1-rc.1"
        }));
    });
    let out = release_cmd(&root, &server);
    assert!(
...
#[test]
fn member_changelog_is_read_from_the_member_directory() {
    let root = temp_git_repo("body-member");
    write_workspace(&root, &[("alpha", "0.1.0"), ("beta", "0.1.0")]);
    commit(&root, "init");
    git(&root, &["tag", "alpha/v0.1.0"]);
    git(&root, &["tag", "beta/v0.1.0"]);
    ...
    mock_lookup_empty(&server, "alpha%2Fv0.1.0");
    mock_lookup_empty(&server, "beta%2Fv0.1.1");
    let _create_alpha = mock_create(&server, "alpha/v0.1.0", 201);
    let create = server.mock(|when, then| {
        when.method(POST)
            .path("/repos/oakoss/oakum/releases")
            .body_includes("\"body\":\"- member fix\\n\"");
        then.status(201).json_body(json!({
            "html_url": "https://github.com/oakoss/oakum/releases/tag/beta/v0.1.1"
        }));
    });
    let out = release_cmd(&root, &server);
    assert!(
...
#[test]
fn a_changelog_blob_that_is_not_utf8_refuses_before_tagging() {
    let root = pending_demo("body-not-utf8");
    fs::write(
        root.join("CHANGELOG.md"),
        b"# Changelog\n\n## 0.1.1\n\n\xe9\xff\xfe\n",
    )
    ...
    let stderr = stderr_of(&out);
    assert!(
        stderr.contains("could not read CHANGELOG.md at ")
            && stderr.contains("not valid UTF-8")
            && stderr.contains("no tag was created")
...
#[test]
fn a_fenced_shell_comment_does_not_end_the_release_body() {
    let root = pending_demo("body-fenced");
    fs::write(
        root.join("CHANGELOG.md"),
        "# Changelog\n\n## 0.1.1 (2026-09-08)\n\nInstall it:\n\n```sh\n# grab the binary\ncargo install oakum\n```\n\nThen run it.\n\n## 0.1.0\n\n- first\n",
    )
    ...
    let server = MockServer::start();
    mock_lookup_empty(&server, "v0.1.1");
    let create = mock_create_with_body(
        &server,
        "Install it:\\n\\n```sh\\n# grab the binary\\ncargo install oakum\\n```\\n\\nThen run it.\\n",
    );
    let out = release_cmd(&root, &server);
    ...
    create.assert();
    assert!(
        !stderr_of(&out).contains("release body:"),
        "{}",
        stderr_of(&out)
    );
}
...
#[test]
fn a_trailing_hard_break_survives_into_the_release_body() {
    let root = pending_demo("body-hard-break");
    fs::write(
        root.join("CHANGELOG.md"),
        "# Changelog\n\n## 0.1.1 (2026-09-08)\n\nline one  \nline two\n",
    )
    ...
    let server = MockServer::start();
    mock_lookup_empty(&server, "v0.1.1");
    let create = mock_create_with_body(&server, "line one  \\nline two\\n");
    let out = release_cmd(&root, &server);
    assert!(
        out.status.success(),
        "{}{}",
...
#[test]
fn a_resumed_tag_with_its_release_already_created_never_reads_its_changelog() {
    let root = pending_demo("body-already-released");
    fs::write(
        root.join("CHANGELOG.md"),
        b"# Changelog\n\n## 0.1.1\n\n\xe9\xff\xfe\n",
    )
...
#[cfg(unix)]
#[test]
fn a_symlinked_changelog_is_named_in_the_fallback_notice() {
    let root = pending_demo("body-symlink");
    fs::write(
        root.join("CHANGELOG.real.md"),
        "# Changelog\n\n## 0.1.1 (2026-09-08)\n\n- behind the link\n",
    )
    .expect("real changelog");
    std::os::unix::fs::symlink("CHANGELOG.real.md", root.join("CHANGELOG.md")).expect("symlink");
    commit(&root, "changelog");
    add_bare_origin(&root);
    let server = MockServer::start();
    mock_lookup_empty(&server, "v0.1.1");
    ...
    let stderr = stderr_of(&out);
    assert!(
        stderr.contains("release body: CHANGELOG.md at ")
            && stderr.contains(" is a symlink to `CHANGELOG.real.md`; using the title"),
        "{stderr}"
    );
}

#[test]
fn a_missing_heading_falls_back_to_the_title_and_names_the_next_step() {
    let root = pending_demo("body-no-heading");
    fs::write(
        root.join("CHANGELOG.md"),
        "# Changelog\n\n## 0.1.0 (2026-09-01)\n\n- first\n",
    )
