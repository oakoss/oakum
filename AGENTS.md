...
Run `mise run setup` after cloning. A fresh clone has no beads database — `.dolt/` and `*.db` are gitignored — so until `bd init` runs, every `.beads/hooks/*` hook exits 3 and skips itself with one line on stderr.

lefthook owns `.git/hooks`: `lefthook.yml` calls `.beads/hooks/*` directly, and `core.hooksPath` must stay unset — `bd init` sets it, which makes git bypass lefthook silently.
