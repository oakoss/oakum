...
- Whether `version` shelling out to `pnpm install --lockfile-only` is the sanctioned exception to the read-only-discovery rule, or whether the lockfile is edited directly. [docs/contributing/invariants.md](../contributing/invariants.md) (and the repository-root `README.md`) say `version` owns the lockfile entries its bumps invalidate but not by what means.
