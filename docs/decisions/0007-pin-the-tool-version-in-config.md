...
**No config is `unverified`, not defaults.** A repository with `.changeset/config.json` and no `_config.toml` has no `tool-version`, so the pin check had nothing to compare and `check` printed nothing and exited 0 (measured in the tsc-files dogfood, `okm-6vf.2`). That is a missed look reported as fine. `check` and `release` refuse when `_config.toml` is absent, as `unverified`, naming `oakum init` and `oakum migrate` as the fix. `status`, `ci pr-status`, and the hidden read commands keep their defaults so a plan can still be inspected before migration; `status` and `ci pr-status` say on stderr that defaults are in effect; the hidden commands say nothing. The writers behind the same gate (`add`, `generate`, `version`, `ci version-pr`) refuse the same way: without a `tool-version` the gate has nothing to hold, and a bump on defaults is the reviewed-commit guarantee failing quietly. `init` and `migrate` are the commands that create the file (`okm-6vf.23`).
...
## Amendment (2026-09-09, npm channel)

The npm package is `@oakoss/oakum`; `oakum` on npm is not this tool. `check` reads the scoped name everywhere it reads the bare one: an exact `@oakoss/oakum` entry in the root `package.json` (an `npm:` alias spec is not an exact version), a versioned `pnpm add` / `pnpm install` / `pnpm dlx` / `npm i` / `npm install` / `npm add` / `npx @oakoss/oakum@x` line in a workflow, and an `npm:@oakoss/oakum` pin in `.mise.toml` / `mise.toml`. The bare name and the scoped name in one `package.json` must agree. A bare `@oakoss/oakum` with no version is `unverified` as an unversioned install, and `pnpm exec oakum` is an invocation, not a pin (`okm-6vf.1`). Before this, an npm-only repository could satisfy `check` only through a pin it did not install from.
