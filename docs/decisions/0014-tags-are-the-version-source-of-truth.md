...
- Bad, because it forces a precondition on every consumer. A shallow clone has no tags, so `check` must verify full history and fail loudly rather than concluding "never released" and computing `0.1.0`. Only two of this repository's six checkout steps set `fetch-depth: 0`; the other four — two more in `ci.yml`, plus `audit.yml` and `codeql.yml` — check out shallow and would need it added before they could run `check`
