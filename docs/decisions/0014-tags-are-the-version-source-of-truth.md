...
- Bad, because it forces a precondition on every consumer. A shallow clone has no tags, so `check` must verify full history and fail loudly rather than concluding "never released" and computing `0.1.0`. As of 2026-08-21 every `actions/checkout` in this repository sets `fetch-depth: 0` (`ci.yml` static-analysis, tests, and secret-scan; `audit.yml`; `codeql.yml`). (When this ADR was written two of six did; [ADR-0025](0025-support-one-rust-version.md) dropped the MSRV job; `okm-cga` filled the remaining three.)
