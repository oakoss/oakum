...
Two caveats keep this a trigger detector rather than the enforcement itself. Clippy [#8849](https://github.com/rust-lang/rust-clippy/issues/8849) confirms the lint cannot see higher-order calls, so passing `Command::new` as a function pointer evades it. And a lint is lint-time where a crate boundary is compile-time. The split remains the real enforcement; the lint is what tells you the trigger fired.
