...
**Amended 2026-08-19: the manifest now declares `[workspace] members = ["probes/plan-no-std"]`, and that member is not a split.** `plan-no-std` ships no code of its own — it mounts `plan`'s sources through one `#[path]` and compiles them under [ADR-0024](0024-no-std-plan-crate.md)'s `#![no_std]`, so `cargo` checks the constraint rather than a reviewer. It publishes nothing, and declares only dependencies the root manifest already lists, so nothing reaches `plan` through a probe that the shipping build does not carry. Neither trigger above has fired; this decision is unchanged. `AGENTS.md` carries the same distinction for anyone reading the tree before the ADRs.
