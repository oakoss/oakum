...
This started as a narrower question — whether a marker earned for the clock counts toward the trigger the same as one earned by I/O. It was not narrower. `Command::new` is the case [structure.md](../contributing/structure.md) names as the trigger rationale, and extraction does no more about it than about the clock.
