...
Two ways out, neither chosen yet: restate the trigger in terms of what library code may *call* rather than what the manifest lists, or add a `clippy.toml` `disallowed-methods` entry for `std::process` and `std::fs` in `plan`, which makes the invariant machine-checked instead of a thing reviewers must remember.
