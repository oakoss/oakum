...
- `tests/prettier_oracle.rs` gates the promise that Prettier leaves generated files alone (`okm-6vf.19`, 2026-09-09): it runs Prettier 3.9.6 over `_schema.json` and the bundled `.changeset/README.md` and asserts both come back byte-identical. `_schema.json` is JSON rather than markdown, but it lands in the same formatter-scanned directory, so the promise covers it too.
- The builtin's sections are Keep a Changelog's six (Added, Changed, Deprecated, Removed, Fixed, Security), in its order. The level picks one by default; a note whose first line is one of those headings picks its own, since `patch` is not always a fix (`okm-6vf.15`, 2026-09-09). Links and attribution stay out of the builtin and go to the template context (`changes`, `repo`).
