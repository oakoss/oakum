...
- Neutral, because no crate split happens until extraction; the constraint lands on how `plan` is written from now, which is the reason to decide now
...
**Mechanised 2026-08-19, ahead of extraction.** `probes/plan-no-std` is a workspace member whose `src/lib.rs` declares `#![no_std]`, `extern crate alloc`, and one `#[path]` mounting `src/plan/mod.rs`. Because `plan` is a directory module, that single path covers every module reachable from `src/plan/mod.rs` with no per-module edit, so a new module cannot drift out of coverage. Three commands compile it: `mise run check` and `check-msrv`, each passing `--workspace --all-targets` — the pair that also holds `plan`'s `#[cfg(test)]` module to `no_std` — and `mise run test`'s `cargo test --workspace --doc`, which compiles it but runs nothing from it. `mise run test`'s first command passes `--exclude plan-no-std` so that `--all-targets` does not report `plan`'s suite a second time. `tests/no_std_probe.rs` asserts the probe stays armed, since a probe that compiles nothing exits 0 and proves nothing.
