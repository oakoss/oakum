...
- What it does on a branch with no bump files that is *intentionally* releaseless — docs-only branches are common, and a hook that cries wolf there gets disabled. **Narrowed, not closed**: `--no-fail` covers the advisory case, but expressing *deliberately releaseless* is the same unsettled marker that blocks `add --empty`, in [specs/bump-files.md](../specs/bump-files.md). This idea cannot be promoted until that settles.
