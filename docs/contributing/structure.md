...
Integration tests build repositories through `tests/support/fixture.rs`; unit tests use `src/test_fixture.rs`. Both write a marker (`.oakum-fixture` / `.oakum-unit-fixture`) into a container under Cargo's `target/tmp` and remove that container on drop. Set `OAKUM_TEST_RETAIN` to keep marked containers for debugging. `mise run test` ends with `scripts/fixture-leak-check.sh`, which fails on marked leftovers and on unmarked `oakum-*` directories (the deliberate `oakum-changeset-foreign` cache is excluded).
