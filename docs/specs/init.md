...
- Generated workflow YAML, with `tool-version` already substituted so a paste matches what `check` later verifies, and `actions/checkout` pinned to the latest GitHub release. The install step is `npm i -g @oakoss/oakum@<tool-version>` for an npm workspace, since `cargo-binstall` is not on `ubuntu-latest` and npm is, and `cargo binstall --no-confirm oakum@<tool-version>` otherwise. For an npm workspace (`package.json` or `pnpm-workspace.yaml` at the root) every job also gets a `pnpm/action-setup` step before oakum runs, pinned the same way: discovery asks pnpm for the packages, and `ubuntu-latest` does not ship it. When `package.json` declares no pnpm version (`packageManager: pnpm@…` or a `devEngines.packageManager` entry named `pnpm`), the step carries `version:` from the pnpm that ran discovery, because the action refuses to run with neither. The check step runs `oakum check --strict`, so a changed package with no covering intent fails the pull request; it skips the version PR (`oakum/version-packages`), where tag drift is the expected state. A missed look is unverified and writes nothing
- What it created, by path, and `replaced .changeset/_schema.json` when a stale schema was overwritten
...
- **Every package private** — `init` writes `private-packages` off, so a workspace whose packages are all unpublishable gets a config that manages nothing, and `check` refuses it. `init` still writes its files and exits zero; the guidance goes to stderr where the config was written, rather than being left to the next command. Both commands print the same sentence, from one string, so they cannot describe the condition differently. See [ADR-0027](../decisions/0027-private-packages-version-opt-in.md).
...
- 2026-09-08: the printed uninstall line names only the files oakum wrote, this run or a byte-identical `README.md` from an earlier one, so a user's own `README.md` is never listed (`okm-6vf.3`) (v0.1)
- 2026-09-09: printed workflow runs `check --strict` (`okm-6vf.11`) (v0.1)
- 2026-09-09: an npm workspace's printed workflow installs through the npm channel (`okm-6vf.8`) (v0.1)
