...
| Converts `.changeset/config.json` → `_config.toml` | Carrying over only keys that still mean something, and naming every key it leaves behind (`config.json` itself is never edited). A silently discarded key is the failure `docs/research/tool-version-pinning.md` records: a stale `prettier` key survived a changesets upgrade with no error and no warning, and formatting changed underneath the user. |
